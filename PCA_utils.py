...
def visualize_generic(data:np.ndarray, rows:int, cols:int, fig_title:str, sub_titles: List):
    """
    ...
    if rows == 1 and cols == 1:
        fig = plt.figure(figsize=(10, 10))
        img_rows = int(np.sqrt(len(data[2:])))
        img_cols = img_rows
        plt.title(fig_title, color="blue", fontweight="bold")
        plt.imshow(data[2:].reshape(img_rows, img_cols), cmap="gray")
        plt.tight_layout()
        plt.show()
        return
